...
interface PassengerManagementContext {
  bookings: any[];
...
/**
 * usePassengerManagement – owns all state and handlers related to the passenger
 * list modal shown from the operations and completed-trips pages.
 *
 * Usage:
 *   const passengerMgmt = usePassengerManagement({ bookings, setTrips, language });
 */
export function usePassengerManagement({
  bookings,
  setTrips,
  language,
}: PassengerManagementContext) {
  ...
  const handleDeletePassenger = async (seatId: string) => {
    ...
    try {
      ...
      if (editingPassengerSeatId && groupSeatIds.includes(editingPassengerSeatId)) {
        setEditingPassengerSeatId(null);
      }
    } catch (err) {
