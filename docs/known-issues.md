...
| `functions/package.json`, `scripts/upload-to-firestore.ts` | `xlsx` HIGH CVE (không có bản vá) | `xlsx` vẫn còn trong Cloud Functions và script | Thay thế hoàn toàn bằng `exceljs` (không có CVE). `xlsx` đã bị xóa khỏi toàn bộ dự án. |
