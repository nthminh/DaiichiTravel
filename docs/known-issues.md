...
| `xlsx` (trong `scripts/`) | HIGH | [GHSA-4r6h-8v6p-xvw6](https://github.com/advisories/GHSA-4r6h-8v6p-xvw6), [GHSA-5pgg-2g8v-p4x9](https://github.com/advisories/GHSA-5pgg-2g8v-p4x9) | Prototype Pollution và ReDoS trong xlsx (tất cả phiên bản). **Không có bản vá.** Hiện chỉ còn dùng trong `scripts/upload-to-firestore.ts` (admin CLI, không liên quan browser). | Script này chạy trong môi trường admin có kiểm soát, không bị expose ra trình duyệt. Rủi ro thực tế rất thấp. Có thể thay bằng `exceljs` nếu cần. |
| `xlsx` (trong `functions/`) | HIGH | Như trên | Prototype Pollution và ReDoS. Phiên bản dùng trong Cloud Functions (`functions/package.json`). | CF chỉ **xuất** (không đọc file do người dùng tải lên), data đầu vào đến từ Firestore (đáng tin cậy). Rủi ro Prototype Pollution trong trường hợp này rất thấp. |

### Rủi ro thực tế

- **xlsx (scripts)**: Chỉ chạy trong môi trường admin khi upload dữ liệu. Không expose ra trình duyệt.
- **xlsx (functions)**: Cloud Function chỉ **tạo** file Excel, không bao giờ parse file do người dùng cung cấp. Data đến từ Firestore. Prototype Pollution không bị trigger trong use-case này.
...
| **xlsx client-side (HIGH CVE)** | 1 | ✅ Đã khắc phục – chuyển sang Cloud Functions |
| **Lỗ hổng `@tootallnate/once`** | 1 (Low) | ⚠️ Chỉ ảnh hưởng Cloud Functions – ưu tiên thấp |
| `xlsx` trong `scripts/` và `functions/` | HIGH | ⚠️ Chưa có bản vá upstream; rủi ro thực tế thấp (chỉ tạo file, không parse input người dùng) |
