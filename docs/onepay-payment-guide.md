...
| **IPN URL** | `https://onepayipn-xxxxxxxx-as.a.run.app` | ⚠️ **Bắt buộc** — URL Cloud Function `onepayIpn` (xem bước 3) |
...
### Bước 3: Lấy IPN URL Của Cloud Function `onepayIpn`

> ⚠️ **Đây là bước quan trọng nhất** — nếu bỏ qua, Cloud Function `onepayIpn` sẽ **không bao giờ được gọi** và giao dịch sẽ không được xác nhận tự động.

Kể từ **Firebase Functions v2**, hàm được triển khai dưới dạng Cloud Run service với URL riêng:

```
https://onepayipn-{PROJECT_HASH}-{REGION_CODE}.a.run.app
```

**Cách lấy URL chính xác:**
1. Vào [Firebase Console](https://console.firebase.google.com) → chọn project → **Functions**
2. Tìm hàm `onepayIpn` trong danh sách
3. Click vào link **"Request"** dưới cột Trigger — đó chính là URL cần dùng
4. Ví dụ: `https://onepayipn-udi5doey4a-as.a.run.app`

> ⚠️ **Lưu ý quan trọng:** URL có dạng `*.a.run.app` (Cloud Run), **KHÔNG** phải `asia-southeast1-{PROJECT_ID}.cloudfunctions.net/onepayIpn` (URL cũ dạng v1). Nếu đăng ký sai URL tại OnePay portal, IPN sẽ không hoạt động.

**Điền vào Settings:**
- Sau khi lấy URL, điền vào trường **IPN URL — Cloud Function onepayIpn (vpc_CallbackURL)** trong Settings → OnePay Vietnam
- URL này sẽ được thêm tự động vào mỗi yêu cầu thanh toán dưới dạng tham số `vpc_CallbackURL`

**Ngoài ra (khuyến nghị thêm):** Đăng ký URL này làm Static IPN URL trong OnePay Merchant Portal Sandbox (`https://mtf.onepay.vn/merchant`) để OnePay luôn gọi đúng URL kể cả khi tham số bị mất.

### Bước 4: Cấu hình Firebase Secret ONEPAY_HASH_KEY

Cloud Function `onepayIpn` cần Firebase Secret `ONEPAY_HASH_KEY` để xác thực chữ ký IPN:

```bash
firebase functions:secrets:set ONEPAY_HASH_KEY
# Nhập Hash Key dạng hex khi được hỏi (phải giống Hash Key trong Settings)
firebase deploy --only functions
```

> Hash Key trong Firebase Secret **phải giống hệt** Hash Key trong Settings. Nếu khác nhau, xác thực chữ ký IPN sẽ luôn thất bại.
...
| **IPN URL** | `https://onepayipn-xxxxxxxx-as.a.run.app` | URL Cloud Function `onepayIpn` (lấy từ Firebase Console) |
...
- [ ] Đã thiết lập webhook IPN — điền **IPN URL** vào trường `IPN URL (vpc_CallbackURL)` trong Settings OnePay và cập nhật Firebase Secret `ONEPAY_HASH_KEY` (xem Bước 3–4 ở [Mục 3](#3-cấu-hình-môi-trường-sandbox-admin))
...
- [ ] Firestore `settings/paymentConfig` có đủ trường: `onepayEnabled`, `onepayMerchant`, `onepayAccessCode`, `onepayHashKey`, `onepayReturnUrl`, `onepayIpnUrl`, `onepayEnvironment`, `onepayGatewayType`
- [ ] **`onepayIpnUrl`** đã được điền đúng URL Cloud Function `onepayIpn` (dạng `*.a.run.app`) — **thiếu trường này IPN sẽ không hoạt động**
- [ ] Firebase Secret `ONEPAY_HASH_KEY` đã được set và deploy functions
...
**Q: Popup QR tự đóng nhưng OnePay không gọi IPN / Cloud Function `onepayIpn` có 0 requests?**
> A: Nguyên nhân phổ biến nhất là trường **IPN URL (vpc_CallbackURL)** chưa được điền trong Settings. Vào Settings → OnePay Vietnam → điền đúng URL Cloud Function `onepayIpn` (lấy từ Firebase Console → Functions, dạng `https://onepayipn-xxxxxxxx-as.a.run.app`). Nếu trường này trống, mỗi yêu cầu thanh toán sẽ thiếu tham số `vpc_CallbackURL` và OnePay không biết phải gửi IPN về đâu.
...
│       ├── onepayIpnUrl: string       ← URL Cloud Function onepayIpn (vpc_CallbackURL)
