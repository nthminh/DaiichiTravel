...
// ---------------------------------------------------------------------------
// Daily Aggregation: keep daily_schedules/{YYYY-MM-DD} in sync with trips
// ---------------------------------------------------------------------------

/**
 * Firestore trigger: whenever a trip document is created, updated, or deleted,
 * mirror the change into the corresponding daily_schedules/{date} document.
 * The daily doc stores a sorted `trips` array so the operations dashboard can
 * fetch exactly N documents (one per day) instead of scanning the entire
 * trips collection.
 */
export const syncTripToDaily = onDocumentWritten(
  { document: 'trips/{tripId}', database: 'daiichi-asia', region: 'asia-southeast1' },
  async (event: any) => {  // eslint-disable-line @typescript-eslint/no-explicit-any
    const db = getFirestore(admin.app(), 'daiichi-asia');
    const tripId = event.params.tripId as string;
    const before = event.data?.before;
    const after = event.data?.after;

    const oldDate: string | null = before?.exists ? ((before.data() as Record<string, unknown>)?.date as string) ?? null : null;
    const newDate: string | null = after?.exists ? ((after.data() as Record<string, unknown>)?.date as string) ?? null : null;

    /** Remove a trip entry from a daily_schedules document. */
    const removeFromDaily = async (date: string) => {
      const ref = db.collection('daily_schedules').doc(date);
      await db.runTransaction(async (tx: FirebaseFirestore.Transaction) => {
        const snap = await tx.get(ref);
        if (!snap.exists) return;
        const trips = ((snap.data()?.trips as unknown[]) ?? []).filter(
          (t: unknown) => (t as Record<string, unknown>).id !== tripId,
        );
        tx.set(ref, { trips, updatedAt: admin.firestore.FieldValue.serverTimestamp(), date }, { merge: true });
      });
    };

    /** Upsert (insert or replace) a trip entry in a daily_schedules document. */
    const upsertInDaily = async (date: string) => {
      const tripData = after!.data() as Record<string, unknown>;
      const ref = db.collection('daily_schedules').doc(date);
      await db.runTransaction(async (tx: FirebaseFirestore.Transaction) => {
        const snap = await tx.get(ref);
        const record: Record<string, unknown> = { id: tripId, ...tripData };
        if (!snap.exists) {
          tx.set(ref, { trips: [record], date, updatedAt: admin.firestore.FieldValue.serverTimestamp() });
        } else {
          const trips = [...((snap.data()?.trips as unknown[]) ?? [])] as Record<string, unknown>[];
          const idx = trips.findIndex((t) => t.id === tripId);
          if (idx >= 0) trips[idx] = record; else trips.push(record);
          trips.sort((a, b) => ((a.time as string) ?? '').localeCompare((b.time as string) ?? ''));
          tx.set(ref, { trips, date, updatedAt: admin.firestore.FieldValue.serverTimestamp() }, { merge: true });
        }
      });
    };

    try {
      // DELETE: trip no longer exists
      if (!after?.exists && oldDate) {
        await removeFromDaily(oldDate);
        return;
      }
      // DATE CHANGED on update: remove from the old day first
      if (oldDate && newDate && oldDate !== newDate) {
        await removeFromDaily(oldDate);
      }
      // CREATE / UPDATE: upsert into the (new) date
      if (newDate) {
        await upsertInDaily(newDate);
      }
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : String(err);
      logger.error('[syncTripToDaily] error', { tripId, oldDate, newDate, error: msg });
    }
  },
);

/**
 * One-time backfill: reads all documents in the `trips` collection and
 * rebuilds every daily_schedules/{date} document from scratch.
 * Call once from the Firebase console (or an admin UI button) after deploying.
 * Trips without a `date` field are skipped.
 */
export const backfillDailySchedules = https.onCall(
  { region: 'asia-southeast1', timeoutSeconds: 540 },
  async (request: https.CallableRequest) => {
    if (!request.auth) {
      throw new https.HttpsError('unauthenticated', 'Must be authenticated to run backfill.');
    }
    const db = getFirestore(admin.app(), 'daiichi-asia');
    ...
    for (const docSnap of tripsSnap.docs) {
      ...
      byDate.get(date)!.push({ id: docSnap.id, ...data });
    }

    // Sort trips within each day and write in batches of 400
    ...
    for (let i = 0; i < entries.length; i += BATCH_SIZE) {
      ...
      for (const [date, trips] of entries.slice(i, i + BATCH_SIZE)) {
        trips.sort((a, b) => ((a.time as string) ?? '').localeCompare((b.time as string) ?? ''));
        batch.set(db.collection('daily_schedules').doc(date), {
          trips,
          date,
    ...
    logger.info('[backfillDailySchedules] completed', { datesWritten, tripsProcessed: tripsSnap.size });
    return { success: true, datesWritten, tripsProcessed: tripsSnap.size };
  },
);
